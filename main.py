...
Automated workflow:
  1. Open temp-mail site (Tab 1) to grab a temporary email.
  2. Open the user's match history URL (Tab 2).
  3. (Optional) Auto-click Review on most recent match.
  4. Auto-fill sign-up form (username + email + password).
  5. Complete onboarding (8 steps).
...
class ChessAutomationApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(
            self.root,
            text=f"v{__version__} — Auto sign-up → onboarding → ready to review",
            font=("Segoe UI", 10),
            bg="#1e1e2e",
            fg="#6c7086",
        ).pack(pady=(0, 12))
        ...
        tk.Label(
            input_frame,
            text="Chess.com Match History URL:",
            font=("Segoe UI", 11),
            bg="#1e1e2e",
            fg="#a6adc8",
            anchor="w",
        ...
        self.url_entry.insert(0, "https://www.chess.com/member/tamle111/games")

        options_frame = tk.Frame(self.root, bg="#1e1e2e")
        options_frame.pack(pady=(10, 5), padx=40, fill="x")

        mail_frame = tk.LabelFrame(
            options_frame,
            text=" Temp-mail Provider ",
            font=("Segoe UI", 10),
            bg="#1e1e2e",
            fg="#a6adc8",
            relief="groove",
        ...
        mail_frame.pack(side="left", fill="y", padx=(0, 10))

        self.provider_var = tk.StringVar(value="temp-mail.org")
        ...
        self.board_var = tk.StringVar(value="Top Left")
        ...
        auto_frame = tk.Frame(self.root, bg="#1e1e2e")
        auto_frame.pack(pady=(8, 5), padx=40, fill="x")

        self.auto_review_var = tk.BooleanVar(value=True)
        tk.Checkbutton(
            auto_frame,
            text="  Auto-click Review (most recent match)",
        ...
        tk.Label(
            auto_frame,
            text="ON = auto-click first Review button  |  OFF = you click Review manually",
        ...
        tk.Label(
            self.root,
            text="Tip: Copy the temp email from Tab 1 if auto-grab fails.",
    ...
    def _on_start_click(self):
        ...
        if not url:
            messagebox.showwarning(
                "Input Required", "Please enter a Chess.com match history URL."
            )
            return
        if "chess.com" not in url.lower():
            if not messagebox.askyesno(
                "URL Check", "The URL doesn't look like Chess.com. Continue anyway?"
            ):
                return
        self.start_button.configure(
            state="disabled", bg="#585b70", text="⏳ RUNNING..."
        )
            ...
            try:
                ...
                if modal_present:
                    ...
                    if email and esel:
                        ...
                        self._update_status(
                            "No temp email — paste from Tab 1.", "#fab387"
                        )
                    ...
                    if psel:
                        if self._human_type_field(
                            tab_game, psel, password, verify=True
                        ):
                    ...
                    self._update_status(
                        f"✅ Signed up!  User: {username}  |  Pass: {password}",
                        "#a6e3a1",
                    )
                    time.sleep(2)
    ...
    def _run_automation(self, match_history_url, provider, board_choice, auto_review):
        try:
            ...
            with sync_playwright() as pw:
                ...
                try:
                    tab_mail.goto(
                        mail_url, wait_until="domcontentloaded", timeout=30000
                    )
                ...
                if temp_email:
                    ...
                    self._update_status(
                        "Couldn't read email. Copy from Tab 1.", "#fab387"
                    )
                ...
                try:
                    tab_game.goto(
                        match_history_url, wait_until="domcontentloaded", timeout=30000
                    )
                ...
                if auto_review:
                    ...
                    self._update_status(
                        "Auto-review OFF — click Review yourself.", "#f9e2af"
                    )
            ...
            self.root.after(
                0, lambda: messagebox.showerror("Error", f"{type(e).__name__}: {e}")
            )
            self.root.after(
                0,
                lambda: self.start_button.configure(
